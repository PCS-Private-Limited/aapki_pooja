  ...
  return (
    <div className={styles.page}>
      <div className={styles.heading}>
        ...
        <p className={styles.sub}>{cartItems.reduce((s, i) => s + i.qty, 0)} items selected</p>
      </div>
      ...
      <div className={styles.layout}>
        ...
        <div className={styles.itemsList}>
          {cartItems.map(item => (
            <div key={item.key} className={styles.itemCard}>
              <div className={styles.itemEmoji} style={{ background: '#fce7f3' }}>
                {item.product.emoji}
              </div>
              ...
              <div className={styles.itemRight}>
                ...
                <div className={styles.qtyControl}>
                  <button className={styles.qtyBtn} onClick={() => updateQty(item.key, -1)}>−</button>
          ...
          <div className={styles.couponBox}>
            ...
            <input className={styles.couponInput} type="text" placeholder="Apply coupon code..." />
        ...
        <div className={styles.summary}>
          ...
          {delivery === 0 && (
            <p className={styles.freeDeliveryNote}>✓ Free delivery on orders above ₹999</p>
          )}
          {delivery > 0 && (
            <p className={styles.freeDeliveryNote}>Add ₹{999 - subtotal} more to get free delivery</p>
          )}
          ...
          {discount > 0 && (
            <div className={styles.savingsBadge}>🎊 You saved ₹{(discount).toLocaleString('en-IN')}!</div>
          )}

          <button className={styles.checkoutBtn} onClick={onNext}>
            Place Order →
          </button>
          <button className={styles.backLink} onClick={onBack}>← Continue Shopping</button>
        </div>
      </div>
    </div>
  )
